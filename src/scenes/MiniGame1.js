...
class MiniGame1 extends Phaser.Scene {
  ...
  create () {
    ...
    this.tutorialBox = this.add.sprite(0, 0, 'DialogueBoxSimp')
      .setOrigin(0, 0)
      .setDepth(150)

    this.tutorialTxt = this.add.text(120, 575, 'This is a matching minigame, click the bushes to find all the matches!', {
      font: '32px Arial',
      color: '#000000'
    })
    this.tutorialTxt.setDepth(160)

    this.tutorialBtn = this.add.image(1200, 740, 'Arrow')
      .setScale(0.7)
      .setDepth(160)
    this.tutorialBtn.setInteractive()
      .on('pointerover', () => {
        this.tutorialBtn.alpha = 0.7
      })
      .on('pointerout', () => {
        this.tutorialBtn.alpha = 1
      })
      .on('pointerup', () => {
        this.tutorialBtn.alpha = 0.7
      })
      .on('pointerdown', () => {
        this.tutorialBtn.alpha = 1
        this.tweens.add({
          targets: [this.tutorialTxt, this.tutorialBox, this.tutorialBtn],
          alpha: {
            getStart: () => 1,
            getEnd: () => 0
          },
